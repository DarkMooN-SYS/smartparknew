...
class _ConfirmPaymentPageState extends State<ConfirmPaymentPage> {
    ...
    try {
      ...
      if (user != null) {
        if (_sufficientFunds) {
          ...
          final notificationTimeUtc =
              parkingTimeUtc.subtract(const Duration(hours: 2));
  ...
  Future<void> _updateWallet() async {
    double price = ((totalPrice - discountedPrice) < 0
        ? 0.00
    ...
    try {
      ...
      if (user != null) {
        await FirebaseFirestore.instance.collection('notifications').add({
          ...
          'type': 'Booking',
          'parkingSlot':
              'Zone : ${widget.selectedZone}, Level : ${widget.selectedLevel}, Row : ${widget.selectedRow}',
          'userId': user.uid,
        });
      }
    } catch (e) {
  ...
  Future<void> _updateSlotAvailability() async {
    try {
      ...
      if (parkingQuerySnapshot.docs.isNotEmpty) {
        ...
        // No parking found
        showToast(
            message: 'No parking found for update: ${widget.bookedAddress}');
      }
      ...
      if (parkingQuerySnapshot.docs.isNotEmpty) {
        ...
        if (zonesQuerySnapshot.docs.isNotEmpty) {
          ...
          // No zones found
          showToast(
              message: 'No zone found for update: ${widget.selectedZone}');
        }
        ...
        if (zoneDocumentSnapshot.exists) {
          ...
          if (levelsQuerySnapshot.docs.isNotEmpty) {
            ...
            // No levels found
            showToast(
                message: 'No level found for update: ${widget.selectedLevel}');
          }
          ...
          if (levelDocumentSnapshot.exists) {
            ...
            if (rowsQuerySnapshot.docs.isNotEmpty) {
              ...
              // No rows found
              showToast(
                  message: 'No row found for update: ${widget.selectedRow}');
            }
          } else {
            // No level found
  ...
  Future<void> getDetails() async {
    ...
    try {
      ...
      DocumentSnapshot document =
          await firestore.collection('vehicles').doc(widget.vehicleId).get();
    ...
    try {
      ...
      if (widget.selectedTime.toLowerCase().contains('am') ||
          widget.selectedTime.toLowerCase().contains('pm')) {
                  ...
                  children: <Widget>[
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(
                          onPressed:
                              _sufficientFunds == true ? _bookspace : _topup,
